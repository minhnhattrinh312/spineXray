...
if __name__ == "__main__":
    ...
    if config["SUP_LOSS"]:
        ...
        check_point = pl.callbacks.model_checkpoint.ModelCheckpoint("classification_task/weights/", filename="ckpt{test_loss:0.4f}",
        ...
        classifier = Classifier(model=model, class_weight=config['CLASS_WEIGHT'],
                            num_classes=config["NUM_CLASS"], learning_rate=config["LEARNING_RATE"])
        check_point = pl.callbacks.model_checkpoint.ModelCheckpoint("classification_task/weights/", filename="ckpt{test_f1:0.4f}",
